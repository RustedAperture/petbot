...
describe("/app/api/stats/user/:userId proxy", () => {
  it("returns 401 without session cookie", async () => {
    const req = new Request("http://localhost/api/stats/user/123");

    const res: any = await userGet(req as any, {
  ...
  it("returns 403 when userId does not match session", async () => {
    ...
    const req = new Request("http://localhost/api/stats/user/999", {
      headers: { cookie: sessionCookie(session) },
    });
  ...
  it("forwards to internal API and returns data on success", async () => {
    const session = { user: { id: "123" } };
    const statsBody = { totalsByAction: { pet: { totalHasPerformed: 42 } } };

    const mockFetch = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify(statsBody), { status: 200 }),
      );
    (global as any).fetch = mockFetch;
    ...
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json).toEqual(statsBody);

    expect(mockFetch).toHaveBeenCalledTimes(1);
    const calledUrl = String(mockFetch.mock.calls[0][0]);
    expect(calledUrl).toContain("/api/stats/user/123");
  });
