...
function Button({
  ...
  variant = "default",
  size = "default",
  // Base UI warns if `nativeButton` is true but the `render` prop does not
  // ultimately render a plain <button>.  In our app we often render links or
  // other components via `render`, so default `nativeButton` to false when a
  // render prop is present unless the caller explicitly overrides it.
  nativeButton,
  render,
  ...props
