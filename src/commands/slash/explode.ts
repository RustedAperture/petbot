...
export const command = {
  data: new SlashCommandBuilder()
    .setName("explode")
    .setDescription("Explodes another user")
    .addUserOption((option) =>
      option
        .setName("target1")
        .setDescription("The user you want to explode")
        .setRequired(true),
    )
    .addUserOption((option) =>
      option
  ...
  async execute(interaction) {
    ...
    await checkUser("explode", author, guild);
    ...
    for (const { user, member } of uniqueTargets) {
      ...
      const container = await performAction("explode", target, author, guild);
