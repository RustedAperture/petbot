...
module.exports = {
	...
	async execute(interaction) {
        ...
        if (!petdata[guild].hasOwnProperty(target.id)) {
            await interaction.reply({ content: 'The user has no pet data', ephemeral: true });
        } else {
            const petEmbed = new EmbedBuilder()
                ...
                .addFields(
                    { name: 'Has been pet', value: `${petdata[guild][target.id]["has_been_pet"]}x`, inline: true },
                    { name: 'Used pet', value: `${petdata[guild][target.id]["has_pet"]}x`, inline: true }
                );
