...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option
                .setName('default_pet')
                .setDescription('The URL for the default pet emoji, used when a user doesnt have one already.')
                .setRequired(false)
        )
	...
	async execute(interaction) {
        ...
        if (log_channel != null) {
            ...
            setupEmbed.addFields({ name: 'Log Channel', value: `<#${log_channel.id}>` })
        }
        if (default_pet != null) {
        ...
        setupEmbed.addFields({ name: 'Triggered by', value: `<@${interaction.member.id}>` })

        log_channel.send({embeds: [setupEmbed]})

        interaction.reply({content: 'Updated Configs. This has been logged.', ephemeral: true})

        await fs.writeFile('data/bot_settings.json', JSON.stringify(botData, null, 2), 'utf-8');
    }
}