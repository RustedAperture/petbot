...
import { useDeleteUserData } from "../../../apps/web/hooks/use-delete-user-data.js";
...
describe("useDeleteUserData", () => {
  ...
  it("exposes initial state: isLoading false, error null", () => {
    const { result, unmount } = renderHook(() => useDeleteUserData());
  ...
  it("success path: returns true, resets isLoading, leaves error null", async () => {
    ...
    await act(async () => {
      returnValue = await result.current.deleteUserData("user-123");
    });
  ...
  it("network error: returns false and surfaces thrown error", async () => {
    ...
    let returnValue: boolean | undefined;
    await act(async () => {
      returnValue = await result.current.deleteUserData("user-999");
    });
  ...
  it("URL-encodes the userId in the request", async () => {
    ...
    const { result, unmount } = renderHook(() => useDeleteUserData());
    await act(async () => {
      await result.current.deleteUserData("user with spaces");
    });
  ...
  it("clears a previous error before a subsequent successful call", async () => {
    ...
    await act(async () => {
      await result.current.deleteUserData("user-123");
    });
